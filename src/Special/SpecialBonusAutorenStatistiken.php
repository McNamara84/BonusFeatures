...
class SpecialBonusAutorenStatistiken extends SpecialPage
{
    private $requiredPoints = 8000;

    function __construct()
    {
        parent::__construct('BonusAutorenStatistiken');
    }
    ...
    function execute($par)
    {
        ...
        try {
            ...
            $this->logInfo('User accessed BonusAutorenStatistiken', [
                'user' => $user->getName(),
                'points' => $userPoints
            ]);
            ...
            $output->addWikiTextAsContent("Diese Seite bietet Statistiken zu den Autoren des Maddraxiversums. Sie basieren auf den Daten des Maddraxikons und werden wöchentlich aktualisiert um den Server nicht zu überlasten. Die Auflistung der Häufigste Autoren enthält eine vollständige Liste aller Autoren. Die beliebtesten Autoren enthalten nur Autoren, die mindestens 5 Romane im Maddraxiversum veröffentlicht haben. Die Durchschnittsbewertung basiert auf den Bewertungen der einzelnen Romane.");
            ...
            $this->logError('Error in BonusAutorenStatistiken: {message}', [
    ...
    public function getTableData($prefix, $page, $statisticType)
    {
        ...
        foreach ($jsonFiles as $file) {
            ...
            foreach ($data as $roman) {
                ...
                $autoren = is_array($roman['text']) ? $roman['text'] : [$roman['text']];
                foreach ($autoren as $autor) {
                    if (!isset($autorenHaeufigkeit[$autor])) {
                        $autorenHaeufigkeit[$autor] = 0;
                    }
                    $autorenHaeufigkeit[$autor]++;
                }

                // Berücksichtige Bewertungen nur für Romane mit mindestens 5 Stimmen
                if ($roman['stimmen'] >= 5) {
                    foreach ($autoren as $autor) {
                        if (!isset($autorenBewertungen[$autor])) {
                            $autorenBewertungen[$autor] = ['sum' => 0, 'count' => 0];
                        }
                        $autorenBewertungen[$autor]['sum'] += $roman['bewertung'];
                        $autorenBewertungen[$autor]['count']++;
                    }
                }
            }
        }
        ...
        if ($type === 'haeufigkeit') {
            arsort($autorenHaeufigkeit);
            $tableData = $autorenHaeufigkeit;
            $headers = ['Autor', 'Anzahl der Romane'];
            $rowCallback = function ($autor, $haeufigkeit) {
                return [$this->createWikiLink($autor), $haeufigkeit];
            };
        } elseif ($type === 'bewertung') {
            // Entferne Autoren mit weniger als 5 bewerteten Romanen für die Bewertungstabelle
            $autorenBewertungen = array_filter($autorenBewertungen, function ($bewertung) {
                return $bewertung['count'] >= 5;
            });

            $durchschnittsBewertungen = [];
            foreach ($autorenBewertungen as $autor => $bewertung) {
                $durchschnittsBewertungen[$autor] = $bewertung['sum'] / $bewertung['count'];
            }
            arsort($durchschnittsBewertungen);
            $tableData = $durchschnittsBewertungen;
            $headers = ['Autor', 'Durchschnittliche Bewertung', 'Anzahl der bewerteten Romane'];
            $rowCallback = function ($autor, $bewertung) use ($autorenBewertungen) {
                return [$this->createWikiLink($autor), number_format($bewertung, 2), $autorenBewertungen[$autor]['count']];
            };
        } else {
    ...
    private function createWikiLink($autor)
    {
        $title = Title::newFromText($autor);
        if ($title !== null) {
            $url = $title->getLocalURL();
            $linkText = htmlspecialchars($autor);
            return "<a href=\"$url\">$linkText</a>";
        }
        return htmlspecialchars($autor);
    }
    ...
    private function getSeriesTable($series)
    {
        ...
        // Tabelle für Häufigste Autoren
        $output .= "=== Häufigste Autoren ===\n";
        ...
        // Tabelle für beliebteste Autoren
        $output .= "=== Beliebteste Autoren ===\n";
    ...
    private function getMaddraxiversumTable()
    {
        ...
        // Container für Häufigste Autoren
        $output .= "=== Häufigste Autoren ===\n";
        ...
        // Container für beliebteste Autoren
        $output .= "=== Beliebteste Autoren ===\n";
